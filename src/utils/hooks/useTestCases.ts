...
import { TestCaseArray, TestCase, ChromeMessage } from '../../types/types';
...
export const useTestCases = () => {
    ...
    const requestTestCases = () => {
        try {
            browserAPI.runtime.sendMessage({ requestTestCases: true })
                .catch(error => {
                    logger.error("Error requesting test cases:", error);
                });
        } catch (error) {
    ...
    const setupTestCaseListener = () => {
        const messageListener = (message: ChromeMessage & { testCase?: TestCase[] }) => {
            if (message.testCase) {
        ...
        try {
            ...
            return () => {
                try {
                    ...
                    logger.error("Error removing message listener:", error);
                }
            };
        } catch (error) {
